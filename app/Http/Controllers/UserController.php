...
class UserController extends Controller
{
    ...
    public function destroy(Request $request, User $user) { abort_if($user->is($request->user()), 422, 'No puedes eliminar tu propio usuario.'); $request->validate(['password' => ['required', 'current_password']]); $old = $user->only('is_active'); $user->update(['is_active' => false]); $user->branches()->detach(); $this->audit('user.deleted', $user, $old, ['is_active' => false, 'deleted' => true]); $user->delete(); return back()->with('success', 'Usuario eliminado y conservado en el historial.'); }
}
