...
{
    public function index(Request $request) { abort_unless($request->user()->can('inventory.view'), 403); $allowed = $request->user()->role === 'administrador' ? null : $request->user()->branches()->pluck('branches.id'); $items = InventoryMovement::with(['product', 'branch', 'user'])->when($allowed, fn($q) => $q->whereIn('branch_id', $allowed))->when($request->search, fn($q,$v) => $q->whereHas('product', fn($p) => $p->where('name','like','%'.$v.'%')->orWhere('internal_code','like','%'.$v.'%')->orWhere('barcode','like','%'.$v.'%')))->latest('movement_date')->latest('id')->paginate(15)->withQueryString(); return view('movements.index', ['items'=>$items]); }
    public function create(Request $request) { abort_unless($request->user()->can('inventory.manage'), 403); $allowed = $request->user()->role === 'administrador' ? null : $request->user()->branches()->pluck('branches.id'); return view('movements.form', ['products' => Product::where('is_active', true)->when($allowed, fn ($q) => $q->whereHas('inventories', fn ($i) => $i->whereIn('branch_id', $allowed)))->orderBy('name')->get(), 'branches' => Branch::where('is_active', true)->when($allowed, fn($q) => $q->whereIn('id', $allowed))->orderBy('name')->get()]); }
    public function store(Request $request)
    {
        ...
        abort_unless($request->user()->role === 'administrador' || Inventory::where('product_id', $data['product_id'])->where('branch_id', $data['branch_id'])->exists(), 403);
        $movement = null;
