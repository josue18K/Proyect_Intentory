...
@section('content')
<div class="module-intro mb-3"><div><span class="eyebrow-dark">Conteo listo para compartir</span><h2>Químicos y compras rápidas</h2><p>Consulta las cantidades actuales en unidades y comparte cada lista directamente por WhatsApp.</p></div></div>
...
<section class="special-stock-group {{ $index === 0 ? 'active' : '' }}" data-stock-panel="{{ $group['key'] }}">
    ...
    <div class="special-stock-list panel">
        ...
        <article class="special-stock-row">
            ...
            <div class="special-stock-dozens"><strong>{{ $product['quantity'] }}</strong><small>{{ $product['quantity'] === 1 ? 'unidad' : 'unidades' }}</small></div>
        </article>
